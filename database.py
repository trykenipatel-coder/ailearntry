...
def init_db():
    ...
    cursor.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS quizzes (
            ...
            allow_jump INTEGER DEFAULT 0,
            nav_lock_seconds INTEGER DEFAULT 7
        );
    ...
    # Migration: add allow_skip, allow_jump, nav_lock_seconds to quizzes
    try:
        cursor.execute("ALTER TABLE quizzes ADD COLUMN allow_skip INTEGER DEFAULT 0")
    except sqlite3.OperationalError:
        pass
    try:
        cursor.execute("ALTER TABLE quizzes ADD COLUMN allow_jump INTEGER DEFAULT 0")
    except sqlite3.OperationalError:
        pass
    try:
        cursor.execute("ALTER TABLE quizzes ADD COLUMN nav_lock_seconds INTEGER DEFAULT 7")
    except sqlite3.OperationalError:
        pass
...
def seed_demo_feature_quiz():
    """Creates a demo quiz showcasing Skip, Jump Navigation, and Nav Lock features."""
    conn = get_db()
    c = conn.cursor()

    existing = c.execute("SELECT quiz_id FROM quizzes WHERE topic = 'Web Development Fundamentals'").fetchone()
    if existing:
        quiz_id = existing["quiz_id"]
        ac_exists = c.execute("SELECT code_id FROM access_codes WHERE code = 'DEMO2027'").fetchone()
        if not ac_exists:
            c.execute("INSERT OR IGNORE INTO access_codes (quiz_id, code, type, status, max_attempts) VALUES (?, 'DEMO2027', 'primary', 'active', 9999)", (quiz_id,))
            conn.commit()
        conn.close()
        return
    ...
    c.execute("""INSERT INTO quizzes (topic, subject, difficulty, created_by, created_by_type, status, allow_skip, allow_jump, nav_lock_seconds)
                 VALUES (?, ?, ?, ?, 'mentor', 'published', 1, 1, 5)""",
              ("Web Development Fundamentals", "Computer Science", "Mixed", mentor_id))
    quiz_id = c.lastrowid

    questions = [
        ("What does HTML stand for?", "Hyper Text Pre Language", "Hyper Text Markup Language", "High Tech Modern Language", "Home Tool Markup Language", "B", "Easy"),
        ("Which CSS property is used to change text color?", "font-color", "text-color", "color", "foreground", "C", "Easy"),
        ("Inside which HTML element do we put the JavaScript?", "<scripting>", "<javascript>", "<js>", "<script>", "D", "Easy"),
        ("Which property is used to change the background color?", "bgcolor", "background-color", "color", "bg-color", "B", "Easy"),
        ("What does CSS stand for?", "Creative Style Sheets", "Cascading Style Sheets", "Computer Style Sheets", "Colorful Style Sheets", "B", "Easy"),
        ("Which is NOT a JavaScript data type?", "String", "Boolean", "Float", "Number", "C", "Medium"),
        ("What is the correct HTML for creating a hyperlink?", "<a url='http://example.com'>Example</a>", "<a href='http://example.com'>Example</a>", "<link href='http://example.com'>Example</link>", "<hyperlink>http://example.com</hyperlink>", "B", "Medium"),
        ("Which method selects an element by its ID?", "getElementById()", "getElement()", "querySelector()", "selectElement()", "A", "Medium"),
    ]
    for q in questions:
        c.execute("INSERT INTO questions (quiz_id, question_text, option_a, option_b, option_c, option_d, correct_answer, difficulty) VALUES (?,?,?,?,?,?,?,?)",
                  (quiz_id, q[0], q[1], q[2], q[3], q[4], q[5], q[6]))
    ...
    if student:
        sid = student["student_id"]
        c.execute("""INSERT OR IGNORE INTO quiz_results
                     (student_id, quiz_id, marks, total_questions, accuracy, topic, difficulty, time_taken, status, date)
                     VALUES (?, ?, 6, 8, 75.0, 'Web Development Fundamentals', 'Mixed', 320, 'Pass', datetime('now'))""",
                  (sid, quiz_id))

    conn.commit()
    print("[Seed] Feature demo quiz created (skip=ON, jump=ON, lock=5s)")
    conn.close()
