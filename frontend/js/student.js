...
let quizState = {
    ...
    accessCode: '', quizData: null, adaptiveMode: true, answeredIds: [],
    adaptiveQueue: [], adaptiveIndex: 0, currentQuestionStart: 0,
    navLockDelay: 7, navLockTimer: null, navLocked: false, navLockAttempts: 0,
    skippedIds: [], allowSkip: false, allowJump: false, navLockSeconds: 7,
};
...
async function checkQuizReEntry() {
    ...
    try {
        ...
        // Always store quiz data (even if already completed — needed for re-attempt)
        if (result.data) {
        ...
        if (result.already_completed) {
            ...
            if (hintEl) {
                hintEl.innerHTML = '<div class="alert alert-warning" style="text-align:left;font-size:0.8rem;margin-bottom:16px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;flex-shrink:0"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> You\'ve already attempted this quiz. Enter <strong>DEMO2026</strong> for unlimited re-attempts.</div>';
                hintEl.style.display = 'block';
            }
            var inputEl = document.getElementById('access-code-input');
            if (inputEl) inputEl.placeholder = 'Enter DEMO2026 for re-attempt';
        }
    } catch (e) {
        console.error('Error loading quiz:', e);
        document.getElementById('code-entry-screen').innerHTML =
            '<div class="alert alert-danger">Failed to load quiz. Please try again.</div>';
    }
}
...
async function validateAndStart() {
    ...
    if (result.success) {
        ...
        document.getElementById('camera-setup-screen').classList.remove('hidden');
    } else {
        if (errorEl) {
...
async function startQuiz() {
    ...
    // Start camera monitoring (proctoring already started in camera setup)
    if (typeof startCameraMonitoring === "function" && proctorSessionId) {
        startCameraMonitoring();
    }

    quizState.questions = data.questions;
    quizState.totalSeconds = quizState.questions.length * 60;
    quizState.adaptiveMode = true;
    quizState.answeredIds = [];
    quizState.adaptiveQueue = [];
    quizState.adaptiveIndex = 0;

    // Read quiz settings
    quizState.allowSkip = data.allow_skip === 1 || data.allow_skip === true;
    quizState.allowJump = data.allow_jump === 1 || data.allow_jump === true;
    quizState.navLockSeconds = data.nav_lock_seconds || 7;
    quizState.skippedIds = [];
    quizState.navLockDelay = quizState.navLockSeconds;
    ...
    // Try adaptive mode
    try {
        const adaptiveResult = await apiRequest('/api/adaptive/start-quiz', 'POST', {
            quiz_id: parseInt(quizState.quizId),
        });
        if (adaptiveResult.success && adaptiveResult.question) {
            quizState.adaptiveQueue = [adaptiveResult.question];
            quizState.adaptiveData = adaptiveResult;
        } else {
            quizState.adaptiveMode = false;
        }
    } catch (e) {
        quizState.adaptiveMode = false;
    }
...
function renderSecurityBar() {
    ...
    if (!bar) return;
    const vCount = quizState.violations || 0;
    const vColor = vCount === 0 ? '#22c55e' : vCount < 3 ? '#f59e0b' : '#ef4444';
    const vLabel = vCount === 0 ? 'Clean' : `${vCount}/3 Warnings`;
    const items = [
        ...
        { label: 'Violations', active: true, color: vColor, extra: ` ${vLabel}` },
    ...
    bar.innerHTML = `
        <div class="security-bar-enhanced">
            ...
            <div class="sec-grid">
                ${items.map(i => `<div class="sec-item active"><span class="dot" style="background:${i.color};box-shadow:0 0 6px ${i.color}80"></span>${i.label}${i.extra || ''}</div>`).join('')}
            </div>
        </div>`;
}
...
function showQuestion() {
    ...
    if (quizState.adaptiveMode && quizState.adaptiveQueue.length > 0) {
        q = quizState.adaptiveQueue[quizState.adaptiveIndex];
        if (!q) return finishQuiz();
        total = quizState.adaptiveQueue.length;
        qNum = quizState.answeredIds.length + 1;
        quizState.currentQuestionStart = Date.now();
    } else {
        q = quizState.questions[quizState.currentIndex];
        if (!q) return finishQuiz();
        total = quizState.questions.length;
        qNum = quizState.currentIndex + 1;
    }

    var progress = (qNum / Math.max(total, 1)) * 100;
    document.getElementById('progress-fill').style.width = Math.min(progress, 100) + '%';
    document.getElementById('question-number').textContent = 'Question ' + qNum;
    document.getElementById('question-difficulty').textContent = q.difficulty;

    // Show adaptive explanation if available
    var explanationEl = document.getElementById('adaptive-explanation');
    if (quizState.adaptiveData && quizState.adaptiveData.explanation) {
        var exp = quizState.adaptiveData.explanation;
        if (explanationEl) {
            explanationEl.innerHTML = '<div style="padding:12px 16px;background:linear-gradient(135deg,#eef2ff,#f0f9ff);border-radius:var(--radius);margin-bottom:16px;border-left:3px solid var(--brand-500)">' +
                '<div style="font-size:0.75rem;font-weight:600;color:var(--brand-700);margin-bottom:6px">Why this question?</div>' +
                '<ul style="margin:0;padding-left:16px;font-size:0.7rem;color:var(--neutral-600);line-height:1.8">' +
                (exp.reasons || []).map(function(r) { return '<li>' + r + '</li>'; }).join('') +
                '</ul>' +
                (exp.recommendation ? '<div style="margin-top:6px;font-size:0.65rem;color:var(--brand-600);font-style:italic">' + exp.recommendation + '</div>' : '') +
                '</div>';
    ...
    qTextEl.innerHTML = '<span>' + (q.text || q.question_text) + '</span>';

    var letters = ['A', 'B', 'C', 'D'];
    var options = q.options || [q.option_a, q.option_b, q.option_c, q.option_d];
    document.getElementById('options-container').innerHTML = options.map(function(opt, i) {
        var selected = quizState.answers[q.question_id || q.id] === letters[i];
        return '<label class="option-item ' + (selected ? 'selected' : '') + '">' +
            '<input type="radio" name="answer" value="' + letters[i] + '" ' + (selected ? 'checked' : '') +
            ' onchange="selectAnswer(' + (q.question_id || q.id) + ', \'' + letters[i] + '\')">' +
    ...
    if (skipBtn) {
        skipBtn.style.display = quizState.allowSkip ? 'inline-flex' : 'none';
    }

    // Question nav grid visibility
    var navGrid = document.getElementById('question-nav-grid');
    if (navGrid) {
        navGrid.style.display = quizState.allowJump ? 'flex' : 'none';
        renderQuestionGrid();
    }

    // Update nav lock with configurable delay
    quizState.navLockDelay = quizState.navLockSeconds || 7;

    // Delayed Navigation Lock — disable nav for N seconds on each new question
    startNavLock();
}
...
function skipQuestion() {
    ...
    if (!q) return;
    var qid = q.question_id || q.id;
    if (!quizState.skippedIds.includes(qid)) {
        quizState.skippedIds.push(qid);
    }
    nextQuestion();
}

function jumpToQuestion(index) {
    if (quizState.navLocked) return;
...
function renderQuestionGrid() {
    var grid = document.getElementById('question-nav-grid');
    if (!grid) return;
    var total = quizState.questions.length;
    var currentIdx = quizState.currentIndex;
    var html = '';
    for (var i = 0; i < total; i++) {
        var q = quizState.questions[i];
        var qid = q.question_id || q.id;
        var answered = quizState.answers[qid] !== undefined;
        var skipped = quizState.skippedIds.includes(qid);
        var isCurrent = i === currentIdx;
        var cls = 'qn-btn';
        if (isCurrent) cls += ' qn-current';
        else if (answered) cls += ' qn-answered';
        else if (skipped) cls += ' qn-skipped';
        html += '<button class="' + cls + '" onclick="jumpToQuestion(' + i + ')" title="Q' + (i+1) + (answered ? ' - Answered' : skipped ? ' - Skipped' : '') + '">' + (i+1) + '</button>';
    }
    grid.innerHTML = html;
}
...
// ═══════════════════════════════════════════════════════════════════════
// DELAYED NAVIGATION LOCK (DNL)
// Disables Next/Previous for N seconds when a new question opens.
// Prevents fast question-skipping and answer searching.
// ═══════════════════════════════════════════════════════════════════════

function startNavLock() {
    var nextBtn = document.getElementById('next-btn');
    var prevBtn = document.getElementById('prev-btn');
    var lockBar = document.getElementById('nav-lock-bar');
    var lockTimer = document.getElementById('nav-lock-timer');

    if (quizState.navLockTimer) clearInterval(quizState.navLockTimer);

    quizState.navLocked = true;
    ...
    var remaining = quizState.navLockDelay;
    if (lockTimer) lockTimer.textContent = remaining;

    quizState.navLockTimer = setInterval(function() {
        remaining--;
        if (lockTimer) lockTimer.textContent = remaining;
        if (remaining <= 0) {
            clearInterval(quizState.navLockTimer);
            quizState.navLockTimer = null;
            enableNavigation();
        }
    }, 1000);
}
...
function handleNavLockAttempt(direction) {
    quizState.navLockAttempts++;
    var lockBar = document.getElementById('nav-lock-bar');
    var lockMsg = document.getElementById('nav-lock-msg');
    if (lockBar) lockBar.style.display = 'flex';
    if (lockMsg) {
        lockMsg.textContent = 'Navigation locked! Please read carefully. (Attempt ' + quizState.navLockAttempts + ')';
        lockMsg.style.color = '#ef4444';
        setTimeout(function() { lockMsg.style.color = ''; }, 1500);
    }

    if (quizState.navLockAttempts >= 5) {
        quizState.violations++;
        quizState.navLockAttempts = 0;
        if (typeof trackViolation === 'function') {
            trackViolation('navigation_lock_abuse', 'Student attempted rapid navigation multiple times');
        }
    }
}

function prevQuestion() {
    if (quizState.navLocked) { handleNavLockAttempt('prev'); return; }
    if (quizState.currentIndex > 0) { quizState.currentIndex--; showQuestion(); }
}

async function nextQuestion() {
    if (quizState.navLocked) { handleNavLockAttempt('next'); return; }
    var q;
    if (quizState.adaptiveMode && quizState.adaptiveQueue.length > 0) {
        q = quizState.adaptiveQueue[quizState.adaptiveIndex];
    } else {
    ...
    if (!q) return finishQuiz();
    var qid = q.question_id || q.id;
    if (!quizState.answers[qid]) { showAlert('Please select an answer before continuing.', 'warning'); return; }

    var responseTime = Math.round((Date.now() - quizState.currentQuestionStart) / 1000);

    if (quizState.adaptiveMode) {
        try {
            var adaptiveResult = await apiRequest('/api/adaptive/answer', 'POST', {
                quiz_id: parseInt(quizState.quizId),
                question_id: qid,
                answer: quizState.answers[qid],
                response_time: responseTime,
                hint_used: false,
                attempt_number: 1,
                confidence: null,
                answered_ids: quizState.answeredIds,
            });

            if (adaptiveResult.success) {
                quizState.answeredIds.push(qid);

                if (adaptiveResult.quiz_complete) {
                    finishQuiz();
                    return;
                }

                if (adaptiveResult.next_question) {
                    quizState.adaptiveQueue.push(adaptiveResult.next_question);
                    quizState.adaptiveIndex++;
                    quizState.adaptiveData = {
                        explanation: adaptiveResult.explanation,
                        als: adaptiveResult.als,
                    };
                    showQuestion();
                    return;
                }
            ...
            console.warn('[Adaptive] API failed, falling back:', e);
        }
    }
...
async function finishQuiz(terminated) {
    ...
    if (quizState.navLockTimer) { clearInterval(quizState.navLockTimer); quizState.navLockTimer = null; }

    // End proctoring
    if (typeof proctorSessionId !== "undefined" && proctorSessionId) {
        try {
            fetch("/api/proctoring/end", {
                method:"POST", headers:{"Content-Type":"application/json"}, credentials:"include",
                body: JSON.stringify({session_id: proctorSessionId})
            });
...
function startTimer() {
    ...
    quizState.timer = setInterval(() => {
        ...
        if (quizState.seconds % 5 === 0) renderSecurityBar();
    }, 1000);
}
...
async function handleViolation(source = 'tab_switch') {
    ...
    if (result.success) {
        quizState.violations = result.warning_count;
        renderSecurityBar();
        showViolationWarning(result.warning_count, result.terminated, source);
    }
}
...
function showViolationWarning(count, terminated, source = 'tab_switch') {
    ...
    if (terminated || count >= 3) {
        showInAppWarning('Quiz terminated due to multiple security violations!', 'danger');
        clearInterval(quizState.timer);
        finishQuiz(true);
    } else {
        const labels = {
        ...
        const msg = labels[source] || 'Violation detected';
        showInAppWarning(`${msg} — Warning ${count}/3. ${remaining} more and quiz terminates.`, 'danger');
    }
}
