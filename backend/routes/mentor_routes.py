...
def create_quiz():
    ...
    allow_skip = 1 if data.get("allow_skip") else 0
    allow_jump = 1 if data.get("allow_jump") else 0
    nav_lock_seconds = int(data.get("nav_lock_seconds", 7))
    ...
    cursor.execute(
        "INSERT INTO quizzes (topic, subject, difficulty, created_by, created_by_type, status, allow_skip, allow_jump, nav_lock_seconds) VALUES (?, ?, ?, ?, 'mentor', 'published', ?, ?, ?)",
        (topic, subject, difficulty, session["user_id"], allow_skip, allow_jump, nav_lock_seconds),
    )
...
def ai_generate_quiz():
    ...
    try:
        ...
        cursor = db.cursor()
        cursor.execute(
            "INSERT INTO quizzes (topic, subject, difficulty, created_by, created_by_type, status, allow_skip, allow_jump, nav_lock_seconds) VALUES (?, ?, 'Mixed', ?, 'mentor', 'published', ?, ?, ?)",
            (topic, subject, session["user_id"], allow_skip, allow_jump, nav_lock_seconds),
        )
        quiz_id = cursor.lastrowid
