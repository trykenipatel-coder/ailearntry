...
def start_quiz(quiz_id):
    ...
    # Randomize question order per student (anti-collaboration)
    # Same student + same quiz = same order (deterministic seed)
    question_list = list(questions)
    random.seed(student_id * 1000 + quiz_id)
    random.shuffle(question_list)

    # Clear any existing violations for this quiz
    db.execute(
...
def submit_quiz():
    ...
    for q in questions:
        ...
        if user_answer.upper() == q["correct_answer"].strip().upper():
            correct += 1
