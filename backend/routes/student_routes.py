...
def start_quiz(quiz_id):
    ...
    quiz_data["allow_skip"] = quiz["allow_skip"] if "allow_skip" in quiz.keys() else 0
    quiz_data["allow_jump"] = quiz["allow_jump"] if "allow_jump" in quiz.keys() else 0
    quiz_data["nav_lock_seconds"] = quiz["nav_lock_seconds"] if "nav_lock_seconds" in quiz.keys() else 7
    questions_data = [
        {
            "id": q["question_id"],
