...
def _test_connection():
    ...
    for model_name in OPENROUTER_MODELS:
        try:
            ...
            print(f"[AI] Model {model_name} timed out (8s)")
            continue
...
def analyze_proctoring_frame(frame_base64, event_history=None):
    """Analyze a camera frame for cheating behaviors.
    Returns dict with detections list and overall risk level."""
    return _fallback_proctoring(event_history)


def _fallback_proctoring(event_history=None):
    """Return no detections — client-side JS handles analysis."""
    return {
        "detections": [],
        "overall_risk": "low",
        "summary": "Client-side analysis active"
    }
