...
from database import init_db, seed_sample_data, get_db, seed_demo_dsa_quiz, seed_demo_scorecards, seed_demo_student_results, seed_demo_python_quiz, seed_python_basics_quiz, seed_adaptive_demo_data, seed_prs_ere_demo_data, seed_demo_interventions, seed_demo_feature_quiz
...
def insight_chat():
    ...
    from backend.services.gemini_service import is_available as ai_available
    if not ai_available():
...
@app.route("/api/proctoring/verify-face-live", methods=["POST"])
def verify_face_live():
    ...
    data = request.get_json()
    quiz_id = data.get("quiz_id")
    live_photo = data.get("photo", "")
    if not live_photo:
        return jsonify({"success": False, "message": "No photo provided"})
    from backend.services.proctoring_service import ProctoringService
    result = ProctoringService().verify_face_live(user_id, quiz_id, live_photo)
    if not result["verified"]:
        from backend.services.proctoring_service import ProctoringService
        ps = ProctoringService()
        sid = ps.start_session(user_id, quiz_id)
        ps.log_event(sid, user_id, quiz_id, "face_mismatch", live_photo[:500], result["confidence"], "")
        ps.update_risk_level(sid)
    return jsonify({"success": result["verified"], "confidence": result["confidence"], "message": result["message"]})
...
def analyze_proctoring_frame():
    ...
    from backend.services.gemini_service import analyze_proctoring_frame as ai_analyze
    result = ai_analyze(frame)
    from backend.services.proctoring_service import ProctoringService
    ps = ProctoringService()
    if session_id and result.get("detections"):
        for det in result["detections"]:
            etype = det.get("type", "unknown")
            confidence = det.get("confidence", 0)
            severity = "high" if confidence >= 0.7 else "medium" if confidence >= 0.4 else "low"
            ps.log_event(session_id, user_id, quiz_id, etype, frame[:500], confidence, "")
        ps.update_risk_level(session_id)
    return jsonify({"success": True, "data": result})
